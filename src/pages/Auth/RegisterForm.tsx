    ...
    <div className={`relative ${fullWidth ? 'col-span-2' : ''}`}>
      {type === 'select' ? (
        <select
          ...
          {...register(id)}
          name={id}
          className='rounded-xl peer w-full ro border-b-1 border-accent text-gray-500 bg-transparent focus:outline-none focus:border-primary placeholder-transparent'
          required
        >
        ...
        <input
          ...
          type={type}
          placeholder={label}
          className='pl-2 rounded-xl peer h-14 w-full border-b-1  border-accent text-accent bg-transparent placeholder-transparent focus:outline-none focus:border-primary'
          required
        />
      )}
      <label
...
const RegisterForm: React.FC = () => {
  ...
  return (
    <section className='w-full bg-background pt-2 font-primary min-h-screen'>
      {/* Main Form Container */}
      <Breadcrumb />
      <div className='mt-50 pb-50 flex flex-col justify-center min-h-screen'>
        {/* Left Column: Welcome Text (outside the box) */}
        <div className='flex md:flex-row space-x-12 justify-center items-center space-y-6 md:space-y-0 md:space-x-12'>
          <div className='flex flex-col items-center md:items-start text-center md:text-left space-y-6'>
            <h2 className='text-5xl font-extrabold text-accent'>Welcome</h2>
            <p className='text-lg text-accent'>Register to your account</p>
          </div>

          {/* Right Column: Registration Form (inside the box) */}
          <div className='md:w-1/2 border-2 border-accent rounded-xl shadow-lg p-12 pb-4 bg-background'>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className='space-y-10'>
                ...
                <div className='space-y-2'>
                  ...
                  <div className='flex items-center'>
                    <div className='flex-shrink-0 h-12 w-12 rounded-full overflow-hidden bg-accent'>
                      {profileImage ? (
                        <img
                ...
                <div className='flex justify-center'>
                  <button
                    className='relative py-2 px-10 text-background text-base font-bold rounded-full overflow-hidden bg-primary transition-all duration-400 ease-in-out shadow-md hover:scale-105 hover:text-accent hover:shadow-lg active:scale-90 before:absolute before:top-0 before:-left-full before:w-full before:h-full before:bg-gradient-to-r before:from-blue-500 before:to-blue-300 before:transition-all before:duration-800 before:ease-in-out before:z-[-1] before:rounded-full hover:before:left-0'
                    type='submit'
                  >
                    Register
                  </button>
