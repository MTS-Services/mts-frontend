...
function MarketPlaceProfile() {
  ...
  return (
    <div>
      <h1>প্রোফাইল তালিকা</h1>
      <button onClick={refetch}>আবার লোড করো</button>

      {data?.profiles?.map((profile) => (
        <div key={profile.id} style={{ marginBottom: "20px", borderBottom: "1px solid #ccc" }}>
          <h2>{profile.profile_name}</h2>
          <p><strong>ডিপার্টমেন্ট:</strong> {profile.department?.[0]?.department_name || "নেই"}</p>

          <h4>প্রমোশন:</h4>
          <ul>
            {profile.profile_promotion.map((promo) => (
              <li key={promo.id}>
                Amount: {promo.promotion_amount}, Clicks: {promo.clicks}, Impressions: {promo.impressions}
              </li>
            ))}
          </ul>
        </div>
